...
class Test_Native_Rule_Engine_Plugin(resource_suite.ResourceBase, unittest.TestCase):
    ...
    def helper_test_pep(self, rules_to_prepend, icommand, strings_to_check_for=['THIS IS AN OUT VARIABLE'], number_of_strings_to_look_for=1):
        ...
        for s in strings_to_check_for:
            ...
            assert number_of_strings_to_look_for == count, 'Found {} instead of {} occurrences of {}'.format(count, number_of_strings_to_look_for, s)
    ...
    def test_api_plugin(self):
        self.helper_test_pep(rule_texts[self.instance_name][self.class_name]['test_api_plugin'], "iapitest", ['pep_api_rs_hello_world_pre -', ', null_value', 'HELLO WORLD', 'pep_api_rs_hello_world_post -', '128'])
